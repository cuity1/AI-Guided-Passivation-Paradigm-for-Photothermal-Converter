...
## 11. Supplementary Technical Data

### 11.1 Component Bill of Materials (BOM)

| Component | Specification | Quantity | Supplier |
|-----------|---------------|----------|----------|
| MCU | STM32F407VG, 168 MHz, 256 KB RAM | 1 | STMicroelectronics |
| Servo Motor (Az) | 50 W, 2.5 N·m, 100:1 gear | 1 | Maxon Motor |
| Servo Motor (El) | 30 W, 1.5 N·m, 150:1 gear | 1 | Maxon Motor |
| Motor Driver | Dual-channel PWM, 5 A max | 1 | Texas Instruments |
| Photoelectric Sensor | Silicon photodiode, 0.5 V/W·m⁻² | 4 | Hamamatsu |
| Rotary Encoder | 2048 PPR, incremental | 2 | Renishaw |
| ADC | 12-bit, 1 MSPS, 16-channel | 1 | Analog Devices |
| Angular Contact Bearing | 25 mm bore, 52 mm OD | 1 | SKF |
| Pillow Block Bearing | 20 mm bore | 1 | SKF |
| Power Supply | 24 V, 5 A regulated | 1 | Mean Well |

### 11.2 Thermal Management
- Operating Temperature Range: -10°C to +50°C
- Motor Thermal Constant: 150 K/W
- Continuous Duty Rating: 50 W (motor + electronics)
- Passive cooling via aluminum heatsinks (500 cm² surface area)
...
- UV Resistance: Polycarbonate enclosure with UV stabilizers

---

## 12. References and Standards

1. ASHRAE Standard 93-2010: Methods of Testing to Determine the Thermal Performance of Solar Collectors
2. ISO 9060:2018: Solar irradiance measurement standards
3. Meeus, J. (1998). Astronomical Algorithms (2nd ed.). Willmann-Bell.
4. Spencer, J. W. (1971). Fourier series representation of the position of the sun. Search, 2(5), 172.
5. NOAA Solar Calculator: https://www.esrl.noaa.gov/gmd/grad/solcalc/

---

## Appendix A: Code Implementation Framework

### A.1 Pseudocode for Main Control Loop
...
void ControlTask(void) {
    ...
    // 1. Read sensor array
    sensor_data_t sensor_data = ReadSensorArray();
    
    // 2. Calculate directional errors
    float delta_az = atan2(sensor_data.I_right - sensor_data.I_left,
                           sensor_data.I_right + sensor_data.I_left);
    float delta_el = atan2(sensor_data.I_upper - sensor_data.I_lower,
                           sensor_data.I_upper + sensor_data.I_lower);
    
    // 3. Update astronomical position (every 60 seconds)
    if (GetSystemTime() - last_astro_update > 60000) {
        solar_position_t solar_pos = CalculateSolarPosition(
            current_date_time, latitude, longitude);
        theta_az_astro = solar_pos.azimuth;
        theta_el_astro = solar_pos.altitude;
        last_astro_update = GetSystemTime();
    }
    
    // 4. Hybrid target calculation
    float theta_az_target = theta_az_astro + K_az * delta_az;
    float theta_el_target = theta_el_astro + K_el * delta_el;
    
    // 5. Get current position from encoders
    float theta_az_current = GetAzimuthPosition();
    float theta_el_current = GetElevationPosition();
    
    // 6. Calculate errors
    float e_az = theta_az_target - theta_az_current;
    float e_el = theta_el_target - theta_el_current;
    
    // Handle azimuth wraparound
    if (e_az > 180.0f) e_az -= 360.0f;
    if (e_az < -180.0f) e_az += 360.0f;
    
    // 7. PID control
    float u_az = PIDControl(&pid_az, e_az);
    float u_el = PIDControl(&pid_el, e_el);
    
    // 8. Saturate velocities
    float v_az = Saturate(u_az, -MAX_VEL_AZ, MAX_VEL_AZ);
    float v_el = Saturate(u_el, -MAX_VEL_EL, MAX_VEL_EL);
    
    // 9. Convert to PWM and output
    uint8_t pwm_az = VelocityToPWM(v_az, MAX_VEL_AZ);
    uint8_t pwm_el = VelocityToPWM(v_el, MAX_VEL_EL);
    
    SetMotorPWM(MOTOR_AZ, pwm_az);
    SetMotorPWM(MOTOR_EL, pwm_el);
    
    // 10. Log data
    LogSystemState(theta_az_current, theta_el_current, 
                   theta_az_target, theta_el_target,
                   sensor_data);
}
```

### A.2 Solar Position Calculation Function

```c
solar_position_t CalculateSolarPosition(datetime_t dt, 
                                        float latitude, 
                                        float longitude) {
    // Calculate Julian Day Number
    float J = CalculateJulianDay(dt);
    
    // Calculate solar declination
    float T = (J - 2451545.0f) / 36525.0f;
    float L0 = 280.46646f + 36000.76983f*T + 0.0003032f*T*T;
    float M = 357.52911f + 35999.05029f*T - 0.0001536f*T*T;
    float C = (1.914602f - 0.004817f*T - 0.000014f*T*T) * sin_deg(M)
            + (0.019993f - 0.000101f*T) * sin_deg(2*M)
            + 0.000029f * sin_deg(3*M);
    float lambda = L0 + C;
    float epsilon = 23.439291f - 0.0130042f*T - 0.00000016f*T*T 
                  + 0.000000504f*T*T*T;
    float delta = asin_deg(sin_deg(epsilon) * sin_deg(lambda));
    
    // Calculate hour angle
    float T_apparent = (J - 2451545.0f) * 36525.0f;
    float E0 = 280.46061837f + 360.98564724f * T_apparent;
    float GHA = E0 + lambda;
    float LHA = GHA + longitude;
    float H = LHA - 180.0f;
    
    // Calculate altitude and azimuth
    float sin_h = sin_deg(latitude) * sin_deg(delta) 
                + cos_deg(latitude) * cos_deg(delta) * cos_deg(H);
    float h = asin_deg(sin_h);
    
    float sin_A = -cos_deg(delta) * sin_deg(H) / cos_deg(h);
    float cos_A = (sin_deg(delta) - sin_deg(latitude) * sin_deg(h)) 
                / (cos_deg(latitude) * cos_deg(h));
    float A = atan2_deg(sin_A, cos_A);
    
    // Atmospheric refraction correction
    float R = 0.96422f / tan_deg(h + 10.3f/(h + 5.11f));
    h += R / 60.0f;
    
    solar_position_t result;
    result.azimuth = A;
    result.altitude = h;
